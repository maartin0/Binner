...
export function OrderImport(props) {
  ...
  /** Fetch the list of parts from the order */
      ...
      await fetchApi("/api/part/import", {
        ...
        if (response.responseObject.status === 200) {
          if (data.errors && data.errors.length > 0) {
            ...
            setPage(1);
            setTotalRecords(data.response.parts.length);
            setTotalPages(Math.ceil(data.response.parts.length / pageSize));
            // mark all parts as selected for import
  ...
  const handleSetPage = async (e, page) => {
    setPage(page);
  };
